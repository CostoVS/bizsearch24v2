...
export function Nav({ statsCount = 6, unreadMessages = 1 }) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-slate-900 text-white shadow-xl border-b border-slate-800" id="global-navbar">
      ...
      <div className="bg-indigo-600 text-[11px] font-bold text-center py-2 px-4 tracking-wide text-indigo-50 flex items-center justify-center gap-2" id="nav-top-banner">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="flex items-center gap-2 sm:gap-4">
            ...
            <Link
              href="/admin"
              className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition-all duration-150 flex items-center gap-2 ${pathname.startsWith("/admin") ? "bg-emerald-600 text-white shadow-lg shadow-emerald-600/20" : "bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700"}`}
