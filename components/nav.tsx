...
import { LogIn, LogOut, LayoutDashboard, ShieldAlert, Sparkles, Menu, X, Search, Newspaper } from "lucide-react";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="flex items-center space-x-3">
              <Link href="/directory" className="w-12 h-12 bg-emerald-600 rounded-xl flex items-center justify-center text-white hover:bg-emerald-700 transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2">
            ...
            <div className="hidden md:flex items-center space-x-6">
              ...
              {!isLoading && !user && (
                <>
                  <Link href="/premium" className="flex items-center text-sm font-medium text-slate-600 hover:text-emerald-600 transition-colors">
        ...
        <div className="fixed inset-0 z-[100] flex">
          ...
          <div className="relative flex w-full max-w-sm flex-col overflow-y-auto bg-white p-6 shadow-2xl animate-in slide-in-from-left">
            <div className="flex items-center justify-between mb-8">
              ...
              {!isLoading && !user && (
                <Link onClick={() => setMobileMenuOpen(false)} href="/login" className="flex items-center px-4 py-4 text-lg font-medium text-emerald-800 bg-emerald-50 rounded-2xl hover:bg-emerald-100 transition-colors mb-2">
              ...
              {!isLoading && user && (
                <>
                  <Link onClick={() => setMobileMenuOpen(false)} href="/dashboard" className="flex items-center px-4 py-4 text-lg font-medium text-emerald-800 bg-emerald-50 rounded-2xl hover:bg-emerald-100 transition-colors mb-2">
                    <LayoutDashboard className="w-5 h-5 mr-3 text-emerald-600" />
                    Dashboard
                  </Link>
                  {user.role === "ADMIN" && (
                    <Link onClick={() => setMobileMenuOpen(false)} href="/admin" className="px-4 py-4 text-lg font-medium text-rose-700 hover:bg-rose-50 rounded-lg transition-colors flex items-center">
                      <ShieldAlert className="w-5 h-5 mr-3" /> Admin
                    </Link>
                  )}
                  <button onClick={() => { setMobileMenuOpen(false); logout(); }} className="flex items-center w-full text-left px-4 py-4 text-lg font-medium text-slate-600 hover:bg-slate-50 rounded-lg transition-colors">
              ...
              <div className="bg-emerald-950 text-white rounded-3xl p-5 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="inline-flex items-center bg-emerald-900/50 text-emerald-400 text-xs font-semibold px-2.5 py-1 rounded-full border border-emerald-800/50">
                    <Sparkles className="w-3 h-3 mr-1.5" /> South Africa Directory
                  </div>
                  <p className="text-white text-sm font-medium mt-2">Connecting Local Businesses</p>
                </div>
              </div>
            </div>
          </div>
