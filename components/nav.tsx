...
export function Nav({ statsCount = 6, unreadMessages = 1 }) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-slate-900 text-white shadow-xl border-b border-slate-800" id="global-navbar">
      ...
      <div className="bg-indigo-600 text-[10px] sm:text-[11px] font-bold text-center py-2 px-4 tracking-wide text-indigo-50 flex flex-col sm:flex-row items-center justify-center gap-1.5 sm:gap-2" id="nav-top-banner">
        ...
        <span className="break-words max-w-full">Claim unclaimed bulk listings to secure access & elevate to Premium for only R199/month!</span>
      </div>
          ...
          <Link href="/" className="flex items-center gap-2 sm:gap-3 transition-transform active:scale-95 shrink-0">
            ...
            <div>
              <div className="flex items-center gap-1 sm:gap-1.5">
                ...
                <span className="bg-slate-800 text-slate-400 text-[8px] sm:text-[9px] font-black px-1.5 py-0.5 rounded tracking-widest">SA</span>
              </div>
              <p className="text-[9px] sm:text-[10px] text-slate-400 font-medium hidden sm:block">South Africa Directory Database</p>
            </div>
          </Link>
